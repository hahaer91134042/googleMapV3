package tcnr6.com.m1701.providers;
...
public class FriendsContentProvider extends ContentProvider {
	private static final String AUTHORITY = "tcnr6.com.m1701.providers.FriendsContentProvider";
...
//		FriendDBHelper friendDbOpenHelper = new FriendDBHelper(getContext(), DB_FILE, null, 1);
		...
		// 檢查資料表是否已經存在，如果不存在，就建立一個。
				...
				if (cursor != null) {
					if (cursor.getCount() == 0) // 沒有資料表，要建立一個資料表。
						mFriendDb.execSQL("CREATE TABLE " + DB_TABLE + " (" + "id INTEGER PRIMARY KEY," + "name TEXT NOT NULL,"
								+ "grp TEXT," + "address TEXT);");
					cursor.close();
				}
